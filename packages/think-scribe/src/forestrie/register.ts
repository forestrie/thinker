...
export type ReceiptStatus =
	{ state: 'pending' } | { state: 'ready'; receipt: Uint8Array; contentType: string | null };
...
export async function registerStatement(
	...
	if (res.status !== 303) throw new ScrapiError(res.status, await problemDetail(res));
...
export async function queryRegistration(
	...
	if (entryMatch) return { state: 'sequenced', receiptUrl: location, entryId: entryMatch[1]! };
...
export async function fetchReceipt(
	...
	if (res.status !== 200) throw new ScrapiError(res.status, await problemDetail(res));
	return {
		state: 'ready',
