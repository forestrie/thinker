...
async function verifyVoucher(standing: StandingEntry, pinnedKeyXY: Uint8Array): Promise<void> {
	if (!standing.voucher || !standing.sealerId || standing.epoch === undefined)
		throw new DelegateError('standing entry is missing its registrar voucher — refusing to bind');
	if (pinnedKeyXY.length !== 64) throw new DelegateError('known sealer key must be 64 bytes x‖y');
	...
	if (!(claims instanceof Map)) throw new DelegateError('registrar voucher claims undecodable');
