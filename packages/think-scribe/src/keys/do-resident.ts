...
async function generateRecord(kek: CryptoKey): Promise<{
	...
	const pair = (await crypto.subtle.generateKey(EC_ALG, true, ['sign', 'verify'])) as CryptoKeyPair;
...
export class DoResidentKeyProvider implements KeyProvider {
	...
	async signingKeyPair(): Promise<CryptoKeyPair> {
		...
		point[0] = 0x04;
		point.set(this.xy, 1);
		const publicKey = await crypto.subtle.importKey('raw', point as BufferSource, EC_ALG, true, [
			'verify'
		]);
	...
	async rotate(epoch: number): Promise<void> {
		if (epoch <= this.currentEpoch)
			throw new Error(`rotate: epoch ${epoch} must exceed current epoch ${this.currentEpoch}`);
