...
if (opt["known-log-key"]) {
  ...
  if (!exported.identity?.publicKeyXY) die("export carries no identity and no --known-log-key given");
...
console.log(`${works.length} work unit(s) ${opt.work ? "selected" : "exported"} for principal ${exported.principal}\n`);
...
for (const work of works) {
  ...
  if (work.state !== "receipted") {
    console.log(`○ ${shortId} — ${work.state}${work.error ? ` (${work.error})` : ""}, not verifiable yet`);
    continue;
  }
  receipted++;
  ...
  try {
    ...
    checks.push({
      name: "user-envelope",
      ok: bound,
      detail: bound ? `signed by ${verified.address}` : `signer ${verified.address} is not the bound principal`,
    });
  } catch (err) {
