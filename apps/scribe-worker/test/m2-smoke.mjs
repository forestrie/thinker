...
async function main() {
	...
	check(
		'delegate-sealing',
		del.ok,
		del.ok ? `sealer ${delBody.sealerId}` : JSON.stringify(delBody)
	);
	...
	check(
		'register 303 accepted',
		reg.ok && !!regBody.statusUrl,
		regBody.contentHash ?? JSON.stringify(regBody)
	);
	...
	for (let i = 0; i < 30; i++) {
		status = await (
			await fetch(`${AGENT}/registration?status=${encodeURIComponent(regBody.statusUrl)}`, {
				headers: AUTH
			})
		).json();
