...
async function main() {
  ...
  check("delegate-sealing", del.ok, del.ok ? `sealer ${(await del.json()).sealerId}` : await del.text());
  ...
  check("work registered", work?.state === "registered", work?.state === "error" ? work.error : work?.contentHash?.slice(0, 16));
  ...
  check("statement has output hash + salt", /^[0-9a-f]{64}$/.test(payloadJson.outputHash) && /^[0-9a-f]{64}$/.test(payloadJson.salt));
  check("statement has leafId", typeof payloadJson.leafId === "string" && payloadJson.leafId.length > 0);

  // Sequencing → receipt → offline verify.
  let status;
  for (let i = 0; i < 30; i++) {
    status = await (
      await fetch(`${AGENT}/registration?status=${encodeURIComponent(work.statusUrl)}`, { headers: AUTH })
    ).json();
  ...
  if (existsSync(cli)) {
    try {
      execFileSync(cli, [
        "verify",
        "--known-log-key", Buffer.from(identity.publicKeyXY, "hex").toString("base64"),
        "--receipt", receiptPath,
        "--payload", join(OUT, "m3-statement.cose"),
